...
model = Sequential()
model.add(CuDNNLSTM(4, return_sequences=True, input_shape=(maxlen, len(chars))))
model.add(CuDNNLSTM(5, return_sequences=False))
model.add(Dense(2, activation='softmax'))

model.compile(loss='categorical_crossentropy', optimizer='rmsprop', metrics=['accuracy'])

model.fit(X, y,validation_split=0.2,epochs=50, callbacks=[WandbCallback()])
